...
// weekKey format: "current:Week 24", "past:Week 23", "daily:2026-06-09:Monday"
...
export default function PayrollPanel() {
  ...
  const [payouts, setPayouts]           = useState({}); // { weekKey:employee -> { paid, id, amount } }
  ...
  if (loading) return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      {[1,2,3].map((i) => <div key={i} className="loading-skeleton" style={{ height: 60, borderRadius: 10 }} />)}
    </div>
  );
    ...
    return (
      <button
        ...
        style={{
          ...
          background: hover ? "rgba(79,216,122,0.05)" : "var(--accent-dim)",
  ...
  return (
    <button
      ...
      style={{
        display: "inline-flex", alignItems: "center", gap: 5,
        padding: "4px 10px", borderRadius: 6, cursor: "pointer",
        background: hover ? "var(--yellow-dim)" : "var(--surface2)",
        border: `1px solid ${hover ? "rgba(246,201,14,0.3)" : "var(--border)"}`,
        color: hover ? "var(--yellow)" : "var(--text-dim)",
...
function Spinner() {
  return (
    <span style={{
      ...
      border: "1.5px solid var(--border)", borderTopColor: "var(--text-muted)",
...
function TableRow({ cells, cols, last }) {
  ...
  return (
    <div
      ...
      onMouseLeave={() => setHover(false)}
      style={{
        display: "grid", gridTemplateColumns: `repeat(${cols || cells.length}, 1fr)`,
        padding: "11px 18px",
...
function Badge({ pos, children }) {
  ...
  return (
    <span style={{
      ...
      background: isServer ? "var(--blue-dim)" : isKitchen ? "rgba(251,146,60,0.12)" : "var(--surface2)",
